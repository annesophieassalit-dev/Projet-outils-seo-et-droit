...
from config import (
    ...
    YELLOW_LEFT, YELLOW_RIGHT, YELLOW_TEXT, BRAND_TAGLINE,
)
...
def draw_multiline_centered(draw: ImageDraw.ImageDraw,
                             ...
                             top_aligned: bool = False) -> None:
    """
    Enveloppe et centre le texte horizontalement (et verticalement si top_aligned=False)
    dans la boîte (box_x1, box_y1, box_x2, box_y2).
    Le retour à la ligne est basé sur la largeur réelle en pixels.
    """
    box_w = box_x2 - box_x1

    # Découpage par paragraphes (sauts de ligne explicites), puis par mots
...
# ─── Pilule dégradée bleue/violette (CTA stories) ───────────────────────────

def draw_gradient_pill(img: Image.Image, draw: ImageDraw.ImageDraw,
                       x1: int, y1: int, x2: int, y2: int,
                       text: str, font: ImageFont.FreeTypeFont,
                       italic: bool = False, pill_radius: int = -1) -> None:
    """Pilule avec dégradé horizontal bleu ciel → lavande (#94b9ff → #e894ff).
...
# ─── Pilule dégradée jaune (bandeaux stories & flash posts) ─────────────────
