  ...
  if (isDark) {
    ...
    const textEls = lines.map((line, i) => {
      ...
      if (matched) {
        ...
        return `<text x="${W/2}" y="${y}" text-anchor="middle" font-size="${fs}" font-family="${FONT}" font-weight="800">`
          + `<tspan fill="${t.ht}">${esc(line.slice(0,lo))}</tspan>`
          + `<tspan fill="${t.ha}">${esc(line.slice(lo, lo+matched.length))}</tspan>`
          + `<tspan fill="${t.ht}">${esc(line.slice(lo+matched.length))}</tspan></text>`;
      }
...
<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}" viewBox="0 0 ${W} ${H}">
  <defs>
    ...
    <linearGradient id="lg" x1="0" y1="0" x2="1" y2="1">
      <stop offset="0%" stop-color="${t.h2}" stop-opacity="0.6"/>
  ...
  // Info slide — fond clair, numéro en cercle, texte centré
  ...
  const textEls = lines.map((line, i) => {
    ...
    if (matched) {
      const lo = line.toLowerCase().indexOf(matched.toLowerCase());
      return `<text x="${W/2}" y="${y}" text-anchor="middle" font-size="${fs}" font-family="${FONT}" font-weight="800">`
        + `<tspan fill="${t.it}">${esc(line.slice(0,lo))}</tspan>`
        + `<tspan fill="${t.ia}">${esc(line.slice(lo, lo+matched.length))}</tspan>`
        + `<tspan fill="${t.it}">${esc(line.slice(lo+matched.length))}</tspan></text>`;
    }
