...
export default function TrialBanner({ isTrialing, trialDaysLeft, trialExpired, noAccess }: TrialBannerProps) {
  if (noAccess || trialExpired) {
    return (
        ...
        <div className="flex items-center gap-2 text-sm">
          ...
          <span>
            {trialExpired
              ...
              : "Activez votre accès pour utiliser l'outil."}{" "}
            Démarrez pour 1€, puis 19€/mois.
          </span>
        </div>
        <Link
          href="/abonnement"
          className="shrink-0 bg-white text-red-600 px-3 py-1 rounded-lg text-xs font-semibold hover:bg-red-50 transition-colors whitespace-nowrap"
        >
          Démarrer pour 1€
        </Link>
      </div>
