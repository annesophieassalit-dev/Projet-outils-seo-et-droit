...
// ─── Pill component (Arimo top / serif accent) ────────────────────────────────
...
export default function StudioPage() {
  ...
  async function generate(seed = variantSeed) {
    ...
    try {
      ...
      setResult("Une erreur est survenue. Vérifiez votre clé API et réessayez.");
    } finally {
      setIsLoading(false);
    }
  }
  ...
  // Parsed views for carousel and TikTok
  ...
  return (
    <div
      ...
      style={{ background: "linear-gradient(160deg, #fdf0f0 0%, #fff6f6 40%, #ffffff 100%)" }}
    >
      {/* ─── Header ─────────────────────────────────────────────────────── */}
        ...
        <div className="max-w-2xl mx-auto px-4 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div className="leading-none">
              <div className="font-montserrat font-black text-[10px] tracking-[0.2em] uppercase text-gray-400">
                VISIBLE ET
              </div>
              <div
                className="font-montserrat font-black text-xl tracking-[0.15em] uppercase"
                style={{ color: "#cf9090" }}
              >
                CONFORME
              </div>
          ...
          <div className="text-right leading-tight">
            <div className="font-arimo text-[11px] font-medium uppercase tracking-widest text-gray-400">
              Studio de Contenu
            </div>
            <div className="font-serif italic text-sm text-gray-600">
              Anne-Sophie Assalit
            </div>
          </div>
        </div>
      ...
      <main className="max-w-2xl mx-auto px-4 py-8 space-y-5">
        ...
        <div className="text-center pt-2 pb-3">
          <h1 className="font-montserrat text-2xl font-black text-gray-900 mb-1">
            Générer mon contenu
          </h1>
              ...
              return (
                <button
                  ...
                  className={[
                    ...
                    active
                      ? "border-[#cf9090] bg-rose-50 shadow-sm"
                ...
                >
                  {Icon && (
                    <Icon
                      className="h-5 w-5"
                      style={{ color: active ? "#cf9090" : color }}
                    />
                  )}
                  <div className="text-center leading-none">
                    <div
                      className={`font-arimo text-xs font-semibold ${
                        active ? "text-[#cf9090]" : "text-gray-700"
                      }`}
                    >
                      {label}
                    </div>
                    {sublabel && (
                      <div className="font-serif italic text-[10px] text-gray-400">
                        {sublabel}
                      </div>
                    )}
                  </div>
        ...
        <section className="bg-white rounded-2xl shadow-sm border border-rose-100 p-5 space-y-5">
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {THEMES.map((t) => (
                <Pill key={t} selected={theme === t} onClick={() => setTheme(theme === t ? "" : t)} size="sm">
                  {t}
                </Pill>
              ))}
            </div>
            ...
            <div className="flex flex-wrap gap-2">
              {OBJECTIVES.map(({ id, label }) => (
                <Pill
                  ...
                  onClick={() => setObjective(objective === id ? "" : id)}
                >
                  {label}
                </Pill>
              ))}
              ...
              {TONES.map(({ id, label, desc }) => (
                <button
                  ...
                  className={[
                    ...
                    tone === id
                      ? "border-[#cf9090] bg-rose-50"
                      : "border-gray-100 hover:border-rose-200",
                  ].join(" ")}
                >
                  <span
                    className={`font-arimo text-sm font-semibold ${
                      tone === id ? "text-[#cf9090]" : "text-gray-700"
                    }`}
                  >
                    {label}
                  </span>
        ...
        <section className="bg-white rounded-2xl shadow-sm border border-rose-100 p-5">
          <div className="flex items-center gap-2 mb-4">
            <span
              className="font-arimo text-xs font-bold uppercase tracking-widest px-2.5 py-1 rounded-full"
              style={{ backgroundColor: "#f5d0d0", color: "#bf7070" }}
            >
              03
            </span>
            <h2 className="font-montserrat font-bold text-sm uppercase tracking-wider text-gray-500">
          ...
          <textarea
            ...
            placeholder="Angle particulier, exemple concret, actualité récente, lien avec une offre en cours, type de praticien ciblé..."
            rows={3}
        ...
        {(result || isLoading) && (
          ...
          >
            {/* Result header */}
            <div
              className="flex items-center justify-between px-5 py-3.5 border-b border-rose-50"
              style={{ background: "linear-gradient(90deg, #fdf0f0, #fff8f8)" }}
            >
              <div className="flex items-center gap-2">
                <span
                  className="font-arimo text-xs font-bold uppercase tracking-wider px-3 py-1 rounded-full text-white"
                  style={{ backgroundColor: "#cf9090" }}
                >
                  {selectedNetwork.label}
                  {selectedNetwork.sublabel ? ` ${selectedNetwork.sublabel}` : ""}
                </span>
                {variantSeed > 0 && (
              ...
              {result && !isLoading && (
                <div className="flex items-center gap-2">
                  ...
                  <button
                    onClick={handleCopy}
                    className={[
                      "flex items-center gap-1.5 px-3 py-1.5 rounded-lg font-arimo text-xs font-medium transition-all",
                      copied
                        ? "bg-[#3aaa78]/10 text-[#3aaa78] border border-[#3aaa78]/30"
                        : "bg-[#cf9090] text-white hover:bg-[#bf7070]",
                    ].join(" ")}
                  >
                    {copied ? (
                      <>
            ...
            <div className="p-5">
              {isLoading && !result ? (
                ...
                <div className="space-y-3">
                  {carouselSlides.map((slide, i) => (
                    <div key={i} className="rounded-xl border border-rose-100 overflow-hidden">
                      <div
                        className="px-4 py-2 font-montserrat text-xs font-black uppercase tracking-wider text-white"
                        style={{ backgroundColor: "#cf9090" }}
                      >
                        {slide.title}
                      </div>
                  ...
                  {isLoading && (
                    <div className="flex items-center gap-2 text-gray-400 px-1">
                      <Loader2 className="h-3.5 w-3.5 animate-spin" style={{ color: "#cf9090" }} />
                      <span className="font-arimo text-xs">Génération en cours…</span>
                    </div>
                  )}
                </div>

              ) : tiktokSections ? (
                /* TikTok section display */
                <div className="space-y-3">
                  {tiktokSections.map((sec, i) => {
                    ...
                    return (
                      ...
                      >
                        <div
                          className="px-4 py-1.5 font-arimo text-xs font-bold uppercase tracking-wider"
                          style={{ color: c.label }}
                        >
                          {sec.section}
                        </div>
                        <div className="px-4 py-3 font-arimo text-sm text-gray-700 whitespace-pre-wrap leading-relaxed border-t border-white/60">
                          {sec.content}
                        </div>
                      </div>
                    );
                ...
                <pre className="whitespace-pre-wrap font-arimo text-sm text-gray-800 leading-relaxed">
                  ...
                  {isLoading && (
                    <span
                      className="inline-block w-0.5 h-[1em] align-middle ml-px animate-pulse"
                      style={{ backgroundColor: "#cf9090" }}
                    />
                  )}
                </pre>
                  ...
                  {network === "threads" && (
                    <span
                      className={`ml-2 font-semibold ${
                        result.length > 500 ? "text-red-500" : "text-[#3aaa78]"
                      }`}
                    >
                      {result.length > 500
                        ? `⚠ dépasse la limite Threads (+${result.length - 500})`
                        : `✓ dans la limite Threads`}
                    </span>
                  )}
                </span>
